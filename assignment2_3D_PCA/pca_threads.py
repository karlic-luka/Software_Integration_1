...
class TextureThreadClass(QThread):
    ...
    def process_PCA_texture(self):
        ...
        try:
            ...
            eigenvectors, _, _ = linalg.svd(
                texture_data.transpose(), full_matrices=False)
            texture_weights = np.dot(texture_data, eigenvectors)
            texture_pca_result['eigenvectors_transposed_flattened'] = eigenvectors.transpose(
            ).flatten()
    ...
    def read_and_prepare_texture_models(self, dir=None):
        ...
        if dir is None:
            ...
            dir = os.path.join(
                os.getcwd(), 'assignment2_3D_PCA', 'Database', 'Texture')
            self.logger.info(f'Using directory: {dir}')
        textures_pngs = [file for file in os.listdir(
            dir) if file.endswith('.png')]
        ...
        data = np.zeros(
            (num_models, model_size[0] * model_size[1] * model_size[2]), dtype=np.float32)
        for i in range(0, num_models):
            data[i, :] = np.float32(imread(textures_pngs[i]) / 255).flatten()
            # +5 so that it reaches 100% after SVD
            self.updated.emit(int((i+1)/(num_models + 5)*100))
...
class GeometryThreadClass(QThread):
    ...
    def process_PCA_geometry(self):
        ...
        try:
            ...
            eigenvectors, _, _ = linalg.svd(
                geometry_data.transpose(), full_matrices=False)
    ...
    def read_and_prepare_geometry_models(self, dir=None):
        ...
        if dir is None:
            dir = os.path.join(
                os.getcwd(), 'assignment2_3D_PCA', 'Database', 'Geometry')
        geometry_obj_files = [file for file in os.listdir(
            dir) if file.endswith('.obj')]
        geometry_obj_files = [os.path.join(dir, file)
                              for file in geometry_obj_files]
        ...
        data = np.zeros((num_models, num_vertices * 3),
                        dtype=np.float32)  # 3 for x, y, z
        for i in range(0, num_models):
